...
object Validator {
    private val PURCHASE_PATTERN = Regex("""\[([^-\[\]]+)-(\d+)]""")
    ...
    fun validatePurchaseInput(input: String): List<Pair<String, Int>> {
        ...
        val matched = matches.joinToString("") { it.value }
        val remaining = input.replace(matched, "").replace(",", "").replace(" ", "")
    ...
    fun validateDate(date: String, fieldName: String = "날짜") {
        require(DATE_PATTERN.matches(date)) {
            "[ERROR] $fieldName 형식이 올바르지 않습니다: $date (YYYY-MM-DD 형식이어야 합니다)"
        }
        ...
        try {
            ...
            require(year in 1900..2100) { "[ERROR] 연도가 유효하지 않습니다: $year" }
            require(month in 1..12) { "[ERROR] 월이 유효하지 않습니다: $month" }
            require(day in 1..31) { "[ERROR] 일이 유효하지 않습니다: $day" }
        } catch (e: NumberFormatException) {
            throw IllegalArgumentException("[ERROR] $fieldName 형식이 올바르지 않습니다: $date")
        }
    }
